//
//  ALSRMStoreTransactionReceiptVerifier.m
//  RMStore
//
...
{    
    ...
    if (!requestData)
    {
        ALSRMStoreLog(@"Failed to serialize receipt into JSON");
        if (failureBlock != nil)
        {
            failureBlock(error);
        }
                  ...
                  failure:(void (^)(NSError *error))failureBlock
{
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (!data)
            {
                ALSRMStoreLog(@"Server Connection Failed");
                NSError *wrapperError = [NSError errorWithDomain:ALSRMStoreErrorDomain code:ALSRMStoreErrorCodeUnableToCompleteVerification userInfo:@{NSUnderlyingErrorKey : error, NSLocalizedDescriptionKey : NSLocalizedStringFromTable(@"Connection to Apple failed. Check the underlying error for more info.", @"RMStore", @"Error description")}];
                if (failureBlock != nil)
                {
            ...
            if (!responseJSON)
            {
                ALSRMStoreLog(@"Failed To Parse Server Response");
                if (failureBlock != nil)
                {
            ...
            {
                ALSRMStoreLog(@"Verifying Sandbox Receipt");
            ...
            {
                ALSRMStoreLog(@"Verification Failed With Code %ld", (long)statusCode);
                NSError *serverError = [NSError errorWithDomain:ALSRMStoreErrorDomain code:statusCode userInfo:nil];
                if (failureBlock != nil)
                {
